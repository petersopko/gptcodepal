...
export const usePromptStore = defineStore('promptStore', () => {
  const promptContexts = ref<PromptContext[]>([
  ...
  const promptSelection = ref<PromptContext>(promptContexts.value[0])

  const updatePromptSelection = (newPromptSelection: PromptContext): void => {
